...
def run_full_model(match: MatchAnalysis) -> MatchAnalysis:
    ...
    # Aceitar partidas que tenham odds reais OU standings reais
    if not match.has_real_odds and not match.has_real_standings:
        raise ValueError(f"Partida {match.match_id} rejeitada: sem odds REAIS nem standings REAIS")
    
    if match.data_quality_score < 0.20:
        raise ValueError(f"Partida {match.match_id} rejeitada: qualidade de dados insuficiente ({match.data_quality_score:.2f} < 0.20)")
...
def run_models_batch(matches: list[MatchAnalysis]) -> list[MatchAnalysis]:
    ...
    print("[MODELS] Partidas com odds REAIS ou standings REAIS serao processadas")
    print()
