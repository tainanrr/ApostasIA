...
# ═══ SAFE PRINT (captura [Errno 22] no Windows / Python 3.14) ═══
...
def _ingest_real_data(analysis_dates: list[str] = None) -> list[MatchAnalysis]:
    ...
    if is_free:
        API_BUDGET = max(api_available - 5, 0)
        print(f"  [FREE] Budget ajustado: {API_BUDGET} calls (reserva=5)")
    else:
        API_BUDGET = min(api_available - 50, 2000)

    if API_BUDGET < 10:
        if is_free:
            print(f"  [FREE] Orcamento baixo ({API_BUDGET}), rodando em modo CACHE-ONLY")
            API_BUDGET = _api_call_count  # Efetivamente 0 calls novas, mas continua com cache
        else:
    ...
    if is_free:
        ...
        print(f"  {len(sorted_leagues)} ligas únicas | [FREE] Apenas cache (API Free nao retorna standings)")
    else:
        max_leagues = min(len(sorted_leagues), config.MAX_STANDINGS_LEAGUES)
    ...
    for lid, count in sorted_leagues[:max_leagues]:
        if not is_free and _api_call_count >= API_BUDGET:
            print(f"  ⚠️  Budget atingido ({_api_call_count}/{API_BUDGET})")
            break
        lname, lcountry, season = league_info.get(lid, ("?", "?", 2025))
        if is_free:
            data = _api_football_request("standings", {"league": lid, "season": season}, cache_only=True)
    ...
    if is_free:
        max_odds = len(fixture_ids)
        print(f"  [FREE] Tentando odds para {max_odds} fixtures (cache hits sao gratis)")
    else:
    ...
    for i, fid in enumerate(fixture_ids[:max_odds]):
        if _api_call_count >= API_BUDGET and not budget_stopped:
            budget_stopped = True
            print(f"  [BUDGET] Limite de API atingido ({_api_call_count}/{API_BUDGET}), continuando apenas com cache...")
        if budget_stopped:
            cached_data = _api_football_request("odds", {"fixture": fid}, cache_only=True)
            response = cached_data.get("response", [])
            oraw = response[0] if response else {}
        else:
    ...
    if is_free:
        max_injuries = 0
        print(f"  [FREE] Pulando lesoes para economizar budget")
    else:
