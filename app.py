...
# Definido no escopo do MÓDULO (Python resolve global antes de builtins).
...
APP_VERSION = "v1.9.4 • 08-abr-2026"
...
def serialize_opportunity(o: ValueOpportunity) -> dict:
    return {
        ...
        "identified_at": getattr(o, 'identified_at', ''),
...
def api_debug_key():
    """Debug: verifica se a API key está carregada e funcional."""
    import requests as req
    import traceback
    key = config.API_FOOTBALL_KEY
    host = config.API_FOOTBALL_HOST
    results = {"key_loaded": bool(key), "key_prefix": key[:8] if key else "EMPTY", "host": host}
    try:
        r = req.get(f"https://{host}/status", headers={"x-apisports-key": key}, timeout=15)
        results["direct_call"] = {"status": r.status_code, "plan": r.json().get("response", {}).get("subscription", {}).get("plan", "?")}
    except Exception as e:
        results["direct_call_error"] = str(e)
    try:
        from data_ingestion import _api_football_request, _get_cached_response
        cached = _get_cached_response("fixtures", {"date": "2026-02-24"})
        results["fixtures_cached"] = cached is not None
        if cached is not None:
            results["cached_results_count"] = len(cached.get("response", []))
        data = _api_football_request("status", {})
        results["_api_football_request_status"] = {"plan": data.get("response", {}).get("subscription", {}).get("plan", "?"), "data_keys": list(data.keys()) if data else "EMPTY"}
        plan_info = _check_api_plan()
        results["_check_api_plan"] = plan_info
    except Exception as e:
        results["internal_error"] = traceback.format_exc()
...
def _supabase_opp_to_frontend(o: dict) -> dict:
    ...
    return {
        ...
        "identified_at": o.get("identified_at", o.get("created_at", "")),
...
def api_check_results():
    ...
    if IS_VERCEL:
        return jsonify({
            "ok": False,
            "error": "No Vercel, use o botão de agendamento (GitHub Actions). A verificação excede o timeout."
        }), 503
    from data_ingestion import fetch_finished_fixtures
    exec_id = supabase_client.log_execution_start("results", "manual")
    ...
    try:
        ...
        supabase_client.log_execution_end(exec_id, "success", result_data)
...
# ═══════════════════════════════════════════
# SCHEDULER CONFIG + EXECUTION HISTORY
# ═══════════════════════════════════════════
...
def api_scheduler_config_update():
    ...
    if config_id not in ("pipeline", "results"):
        return jsonify({"ok": False, "error": "id deve ser 'pipeline' ou 'results'"}), 400

    ok = supabase_client.update_scheduler_config(
        config_id=config_id,
...
def api_scheduler_history():
    """Retorna últimas execuções (pipeline e results)."""
    pipeline_hist = supabase_client.get_execution_history("pipeline", limit=3)
    results_hist = supabase_client.get_execution_history("results", limit=3)
    return jsonify({
        "ok": True,
        "pipeline": pipeline_hist,
...
def api_scheduler_trigger():
    """Dispara GitHub Actions workflow para execução manual via Vercel."""
    from flask import request
    data = request.get_json(silent=True) or {}
    action = data.get("action")
    days = data.get("days", 2)

    if action not in ("pipeline", "results"):
        return jsonify({"ok": False, "error": "action deve ser 'pipeline' ou 'results'"}), 400

    github_token = os.environ.get("GITHUB_TOKEN", "")
    github_repo = os.environ.get("GITHUB_REPO", "")

    if not github_token or not github_repo:
        return jsonify({"ok": False, "error": "GITHUB_TOKEN e GITHUB_REPO não configurados nas variáveis de ambiente"}), 500
    ...
    try:
        ...
        if resp.status_code in (204, 200):
            return jsonify({"ok": True, "msg": f"Workflow disparado: {action} ({days} dias)"})
        else:
            return jsonify({"ok": False, "error": f"GitHub API retornou {resp.status_code}: {resp.text}"}), 500
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


# ═══════════════════════════════════════════
# FILTER VIEWS (Visões de filtro na nuvem)
# ═══════════════════════════════════════════

@app.route("/api/filter-views")
def api_get_filter_views():
    views = supabase_client.get_filter_views()
    return jsonify({"ok": True, "views": views})


@app.route("/api/filter-views", methods=["POST"])
def api_save_filter_view():
    from flask import request
    data = request.get_json(silent=True) or {}
    required = ("id", "name", "target", "state")
    if not all(data.get(k) for k in required):
        return jsonify({"ok": False, "error": "Campos obrigatórios: id, name, target, state"}), 400
    view = {
        "id": data["id"],
        "name": data["name"],
        "target": data["target"],
        "state": data["state"],
        "created_at": data.get("createdAt") or data.get("created_at") or datetime.now(config.BR_TIMEZONE).isoformat(),
    }
    ok = supabase_client.save_filter_view(view)
    return jsonify({"ok": ok})


@app.route("/api/filter-views/<view_id>", methods=["DELETE"])
...
def _scheduled_run():
    ...
    now = datetime.now(config.BR_TIMEZONE)
    print(f"\n[SCHEDULER] Execução automática iniciada — {now.strftime('%d/%m/%Y %H:%M:%S')}")
    try:
        with app.app_context():
            run_engine()
        print(f"[SCHEDULER] Execução concluída com sucesso!")
    except Exception as e:
        print(f"[SCHEDULER] Erro na execução automática: {e}")
        import traceback
        try:
            traceback.print_exc()
        except OSError:
...
def _setup_scheduler():
    """Configura o agendador para rodar o pipeline diariamente."""
    scheduler = BackgroundScheduler(daemon=True)

    for hour in SCHEDULER_HOURS:
        scheduler.add_job(
            _scheduled_run,
            CronTrigger(hour=hour, minute=0, timezone=config.BR_TIMEZONE),
            id=f"daily_run_{hour:02d}h",
            name=f"Pipeline diário {hour:02d}:00 (Brasília)",
...
if __name__ == "__main__":
    ...
    next_runs = [f"{h:02d}:00" for h in SCHEDULER_HOURS]
    print(f"  Execução automática: diária às {', '.join(next_runs)} (Brasília)")
