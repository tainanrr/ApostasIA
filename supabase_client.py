...
def get_pending_opportunities() -> list[dict]:
    ...
    try:
        ...
        try:
            import traceback
            with open("_error_log.txt", "a", encoding="utf-8") as _f:
                _f.write(f"\n{'='*60}\n[GET_PENDING ERROR] {e}\n")
...
def get_opportunities_by_dates(date_from: str, date_to: str) -> list[dict]:
    ...
    try:
        ...
        try:
            import traceback
            with open("_error_log.txt", "a", encoding="utf-8") as _f:
                _f.write(f"\n{'='*60}\n[GET_OPPS_BY_DATES ERROR] {date_from}->{date_to}: {e}\n")
                traceback.print_exc(file=_f)
        except Exception:
            pass
        return []
...
# ═══════════════════════════════════════════════════════════════
#  FILTER VIEWS — Visões de filtro persistentes na nuvem
#  Tabela: filter_views (id text PK, name text, target text,
#          state jsonb, created_at timestamptz)
# ═══════════════════════════════════════════════════════════════

def _ensure_filter_views_table():
    """Cria a tabela filter_views se não existir (via RPC SQL)."""
    sb = get_client()
    if not sb:
        return False
    try:
        sb.table("filter_views").select("id").limit(1).execute()
        return True
    except Exception:
        try:
            import requests as _req
            url = f"{config.SUPABASE_URL}/rest/v1/rpc/exec_sql"
            _req.post(url, json={"query": """
                ...
                ALTER TABLE filter_views ENABLE ROW LEVEL SECURITY;
                CREATE POLICY "allow_all" ON filter_views FOR ALL USING (true) WITH CHECK (true);
            """}, headers={
...
def get_filter_views() -> list[dict]:
    """Retorna todas as visões de filtro salvas."""
    sb = get_client()
    if not sb:
        return []
    try:
        result = (
            sb.table("filter_views")
            .select("*")
        ...
        print(f"[SUPABASE] Erro ao buscar filter_views: {e}")
        return []


def save_filter_view(view: dict) -> bool:
    """Salva (upsert) uma visão de filtro."""
    sb = get_client()
    if not sb:
        return False
    try:
        sb.table("filter_views").upsert(view).execute()
        return True
    except Exception as e:
        print(f"[SUPABASE] Erro ao salvar filter_view: {e}")
        return False


def delete_filter_view(view_id: str) -> bool:
    """Remove uma visão de filtro pelo ID."""
    sb = get_client()
    if not sb:
        return False
    try:
        sb.table("filter_views").delete().eq("id", view_id).execute()
        return True
    except Exception as e:
        print(f"[SUPABASE] Erro ao deletar filter_view: {e}")
        return False
...
def get_scheduler_config():
    """Retorna configuração do scheduler (pipeline + results)."""
    client = get_client()
    if not client:
        return []
    try:
        result = client.table("scheduler_config").select("*").execute()
        return result.data or []
    except Exception as e:
        print(f"[SUPABASE] Erro ao buscar scheduler_config: {e}")
        return []


def update_scheduler_config(config_id: str, enabled: bool = None, hours: list = None, days_range: int = None):
    """Atualiza configuração de um scheduler (pipeline ou results)."""
    client = get_client()
    if not client:
        return False
    try:
        ...
        print(f"[SUPABASE] Erro ao atualizar scheduler_config: {e}")
        return False
...
def log_execution_start(exec_type: str, trigger: str = "manual") -> str:
    """Registra início de uma execução. Retorna o ID do registro."""
    client = get_client()
    if not client:
        return ""
    try:
        result = client.table("execution_history").insert({
            "type": exec_type,
            "trigger": trigger,
            "status": "running",
        }).execute()
        ...
        print(f"[SUPABASE] Erro ao registrar execução: {e}")
        return ""


def log_execution_end(exec_id: str, status: str = "success", details: dict = None, error_message: str = None):
    ...
    try:
        ...
        client.table("execution_history").update(data).eq("id", exec_id).execute()
    except Exception as e:
        print(f"[SUPABASE] Erro ao atualizar execução: {e}")


def get_execution_history(exec_type: str = None, limit: int = 3):
    """Retorna últimas execuções. Se exec_type fornecido, filtra por tipo."""
    client = get_client()
    if not client:
        return []
    try:
        query = client.table("execution_history").select("*").order("started_at", desc=True).limit(limit)
        if exec_type:
            query = query.eq("type", exec_type)
